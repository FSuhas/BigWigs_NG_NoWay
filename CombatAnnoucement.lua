...
-- 		["Announces in chat channel les casts instantanés importants."] = true,
-- 		["Combat Announcement"] = true,
-- 		["combatannouncement"] = true,
-- 	}
-- end)
...
if class == "WARRIOR" then
	BigWigsCombatAnnouncement.consoleOptions = {
		...
		args = {
			...
			broadcastraid_warning = {
				type = "toggle",
				name = "Broadcast Raid Warning",
				order = 99,
				desc = "Toggle broadcasting the messages to the Raid channel.",
				get = function()
					return BigWigsCombatAnnouncement.db.profile.broadcastraid_warning
				end,
				set = function(v)
					BigWigsCombatAnnouncement.db.profile.broadcastraid_warning = v
				end,
			},
			broadcastbg = {
				type = "toggle",
...
function BigWigsCombatAnnouncement:IsBroadcasting()
	if self.db.profile.broadcastsay or self.db.profile.broadcastparty or self.db.profile.broadcastraid or self.db.profile.broadcastbg or self.db.profile.broadcastraid_warning then
		return true
	end
end
...
function BigWigsCombatAnnouncement:AnnounceAbility(msg)
	...
	if self.db.profile.broadcastraid_warning and UnitInRaid("player") then
	...
	for optionname, translatedname in pairs(SpellTranslation) do
		if self.db.profile[optionname] == true then
			if name == translatedname then
				local CombatAnnouncementString = nick .. ": " .. name .. " sur " .. '%T'
